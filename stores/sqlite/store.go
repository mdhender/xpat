...
func Create(path string, ctx context.Context) error {
	...
	store := &Store{path: path, db: db, ctx: ctx, q: New(db)}
	if err := store.q.InsertUser(ctx, InsertUserParams{
		ID:        1,
		Name:      "mike",
		Email:     "mike@example.com",
		CreatedBy: 1,
	}); err != nil {
		log.Printf("db: create: failed to create default user\n")
		return err
	}
	log.Printf("db: create: created default user\n")
